...
//
// const {FastTrackSale, FastTrackRequest, FastTrackBestMatch, FastTrackFollowup, FastTrackReminder, VehicleListing, Customer, User} = db;
//
...
//         body("vehicle_type").isString().notEmpty(),
//         body("vehicle_make").isString().notEmpty(),
//         body("vehicle_model").isString().notEmpty(),
//         body("grade").isString().notEmpty(),
//         body("manufacture_year").isInt({ min: 1900, max: new Date().getFullYear() }),
//         body("mileage_min").isInt({ min: 0 }),
//         body("mileage_max").isInt({ min: 0 }).custom((value, { req }) => value >= req.body.mileage_min),
//         body("no_of_owners").isInt({ min: 0 }),
//         body("price_from").isFloat({ min: 0 }),
//         body("price_to").isFloat({ min: 0 }).custom((value, { req }) => value >= req.body.price_from),
//     ]),
...
//         }
//         try {
//             const { directRequestId, vehicleId } = req.params;
