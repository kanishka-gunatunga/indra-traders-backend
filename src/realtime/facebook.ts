...
export async function handleFacebookMessage(psid: any, received_message: any) {
    ...
    const botResult = await processBotMessage(psid, inputForAi);

    let finalResponseEnglish = "";

    if (typeof botResult === 'object' && botResult.type === 'handoff'){
        await session.update({status: 'queued', priority: 1});
        finalResponseEnglish = botResult.content;
    } else {
        finalResponseEnglish = (typeof botResult === 'string') ? botResult : botResult.content;
    }

    let finalResponseUser = finalResponseEnglish;
    if (session.language && session.language !== 'en') {
        finalResponseUser = await TranslateService.translateText(finalResponseEnglish, session.language);
    }

    await db.ChatMessage.create({
        chat_id: psid,
        sender: session.status === 'queued' ? "system" : "bot",
